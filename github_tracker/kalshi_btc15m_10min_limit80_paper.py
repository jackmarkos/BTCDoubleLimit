...
# Railway: python -u kalshi_btc15m_10min_limit80_paper.py
# For restart persistence set the model-specific state environment variable to a volume path.
# Railway deployment refresh
"""Paper strategy: place YES and NO limits at 45 cents. If exactly one side is filled at 10 minutes, replace the missing-side order with a 80c limit, then hold to settlement.

Run in VS Code: python kalshi_btc15m_10min_limit80_paper.py
...
STATE_PATH = Path(
    os.getenv('PAPER_10MIN_LIMIT80_STATE_PATH')
    or Path(__file__).with_name('kalshi_btc15m_10min_limit80_state.json')
)
